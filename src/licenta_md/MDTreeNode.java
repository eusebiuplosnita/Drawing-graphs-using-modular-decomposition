...
public class MDTreeNode extends RootedTreeNode {
    ...
    /**
     * @return the DEF_COMP_NUM
     */
    public static int getDEF_COMP_NUM() {
        return DEF_COMP_NUM;
    }

    /**
     * @return the DEF_TREE_NUM
     */
    public static int getDEF_TREE_NUM() {
        return DEF_TREE_NUM;
    }
	...
	private static final int DEF_COMP_NUM = -1;
	...
	private static final int DEF_TREE_NUM = -1;
	...
	protected boolean isFullyMarked() {
		return (getNumMarks() == getNumChildren());
	}
	...
	protected void clearTreeNumForSubtree() {
		setTreeNumForSubtree(getDEF_TREE_NUM());
	}
	...
	protected void clearCompNumForSubtree() {
		setCompNumForSubtree(getDEF_COMP_NUM());
	}
	...
	protected void addSplitMark(SplitDirection splitType) {
		...
		if (this.getSplitType() == splitType) {
			return;
		} else if (this.getSplitType() == SplitDirection.NONE) {
	...
	private boolean isSplitMarked(SplitDirection splitType) {
		return (this.getSplitType() == SplitDirection.MIXED ||
				this.getSplitType() == splitType);
	}
	 ...
	 */
	public boolean isALeaf() { return false; }
	...
	protected void removeDegenerateDuplicatesFromSubtree() {
		...
		while(currentChild != null) {
			...
			if (currentChild.getType() == getType() && getType().isDegenerate()) {					
	...
	protected void clearAll() {
		...
		compNumber = getDEF_COMP_NUM();
		treeNumber = getDEF_TREE_NUM();
		numMarks = 0;
		splitType = SplitDirection.NONE;
	...
	public String toString() {
		...
		String result = "(" + getType() + ", numChildren=" + getNumChildren();
