 ...
 * of a graph.
 */
public class RecSubProblem extends RootedTreeNode {
	...
	private void determineLeftCoCompFragments() {
		...
		int lastCompNum = MDTreeNode.getDEF_COMP_NUM();
		
		while (current.getFirstChild() != pivot) {
			int currentCompNum = ((MDTreeNode)current.getFirstChild()).getCompNumber();
			if (lastCompNum != MDTreeNode.getDEF_COMP_NUM() && lastCompNum == currentCompNum) {
